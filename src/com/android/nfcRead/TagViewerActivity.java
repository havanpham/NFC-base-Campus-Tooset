...
public class TagViewerActivity extends Activity {
	...
	// ID of special tag
	private final String ID_ROM = "iRsHust";
		...
		if (records != null) {
			for (NdefRecord record : records) {
				if (UriRecord.isUri(record)) {
					...
					int lenth = ID_ROM.length();
					if(count==0 && TextRecord.parse(record).getText().length()>lenth && TextRecord.parse(record).getText().substring(0, lenth).equals(ID_ROM)){
						isRomSchedule=true;
